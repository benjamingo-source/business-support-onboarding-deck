...
export const overviewSlides: OverviewSlide[] = [
  ...
  },
  {
    ...
    bullets: [
      '🛠️ **BizTech** — owns the systems (Salesforce, CPQ, integrations) and implements policy changes. Escalate for: genuine system bugs, data issues, broken automations. Channel: **#ask-biztech** (check spelling).',
      '📊 **RevOps / SalesOps (Business Partners)** — policy-adjacent: reports, account owner changes, territory queries, recognition thresholds. Escalate for: ARR/attainment disputes, threshold config, policy grey areas.',
      '🗺️ **Territory S&O** — account hierarchies and company structures in Salesforce.',
      '🧠 **Customer Intelligence** — BigBrain views and customer data / metadata (the Espresso tool).',
      '💳 **Payments (Billing Dev)** — subscriptions lifecycle, CC bugs, payment infrastructure, quote calculation.',
      '🧾 **Finance Controllers & Finance Billing** — Controllers validate financial decisions and approve unusual contract terms; Billing owns billing entities and invoice changes. Their yes = go.',
      '🚀 **Deal Acceleration** — non-standard / out-of-policy deals that need an exception. Send here when coverage is unclear.',
      '💬 **CX** — platform support for customers (we redirect; we are business support, not platform support). **Sales Desk** — the partners-only version of us.',
  ...
  },
  {
    ...
    bullets: [
      '🧭 **Three questions before touching anything:** *The ARR question* — what was account ARR before, what is it after, what did this opp add? *The CPQ rule* — if the problem is CPQ, the problem is the contract; start there. *The trust rule* — doubt the ticket description, screenshots tell the truth; translate the rep\'s words into system terms first.',
      '🌍 **Regional awareness** — reps outside Tel Aviv, London, and New York usually had less enablement. When they say "contract" they rarely mean the Salesforce object; when they say "correction opp" they may mean something else. Translate before you diagnose.',
      '🔒 **Sensitive territory** — recognition issues affect rep pay. Be precise, document everything, never override without a paper trail. When policy says no, say no and reference the policy; they can escalate above you, and your documentation protects your position.',
